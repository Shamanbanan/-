...
async function getUniqueNameVariationCodeItems() {
  ...
  try {
    ...
    const dataList = Array.from(uniqueItems).map(item => {
...
async function searchItems(searchTerms) {
  ...
  return dataList.filter(data => {
    ...
    const matchCount = searchTerm.split("").reduce((count, char) => {
      return count + (name.includes(char) ? 1 : 0);
    }, 0);
    return matchCount === searchTerm.length;
  });
}
...
nameInput.addEventListener("input", async function () {
  clearTimeout(searchTimeout); // отменяем предыдущий запрос
  autocompleteList.innerHTML = "";
  if (!this.value) return;
  const searchTerms = this.value.toLowerCase().split(" ");
  searchTimeout = setTimeout(async () => { // добавляем задержку перед новым запросом
    const filteredData = await searchItems(searchTerms);
    const sortedData = filteredData.sort((a, b) => {
      const aMatchCount = a.name
        .toLowerCase()
        .split("")
        .reduce((count, char) => {
          return count + (searchTerms.includes(char) ? 1 : 0);
        }, 0);
      const bMatchCount = b.name
        .toLowerCase()
        .split("")
        .reduce((count, char) => {
          return count + (searchTerms.includes(char) ? 1 : 0);
    ...
    sortedData.forEach((item) => {
      const option = new Option(`ВИ: ${item.variation} (Код: ${item.code})`, item.name);
      option.setAttribute('data-variation', item.variation);
